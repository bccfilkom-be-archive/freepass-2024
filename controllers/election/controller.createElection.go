...
func CreateElection(c *gin.Context) {
	...
	electionData.MakeByAdminID = user.ID

	election, err := electionRepositorys.CreateNewElection(electionData)
	if err != nil {
		res := schemas.ResponeData{Error: true, Message: "Failed to create election, Something went wrong", Data: nil}
		c.JSON(http.StatusInternalServerError, res)
		return
	}
